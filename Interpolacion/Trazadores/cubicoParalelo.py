...
def cubicoNatural(n, x, y):
    ...
    for i in range(len(misXs)):
        ...
        k = (x.index(misXs[i]) - 1) * 4
        for j in range(0, 3):
            fila[k + j] = (3 - j) * pow(misXs[i], exp)
            exp -= 1
        exp = 2

        for j in range(0, 3):
            fila[k + j + 4] = -(3 - j) * pow(misXs[i], exp)
            exp -= 1
        filas.append(fila)
    arregloFilas = comm.gather(filas)
    if rank == 0:
    ...
    for i in range(len(misXs)):
        ...
        k = (x.index(misXs[i]) - 1) * 4
        for j in range(0, 2):
            fila[k + j] = (6 - 4 * j) * pow(misXs[i], exp)
            exp -= 1
        exp = 1

        for j in range(0, 2):
            fila[k + j + 4] = -(6 - 4 * j) * pow(misXs[i], exp)
            exp -= 1
        filas.append(fila)
    arregloFilas = comm.gather(filas)
    if rank == 0:
        ...
        xDistribuidas = repartirEquitativamente(2, [x[0], x[-1]], 0)
    misXs = comm.scatter(xDistribuidas, root=0)
    filas = []
    exp = 1
    # Etapa 4
    if len(misXs) > 0:
        if x.index(misXs[0]) != 0:
            for i in range(len(misXs)):
                fila = [0.0] * (n * 4 + 1)
                k = (x.index(misXs[i]) - 1) * 4
                for j in range(0, 2):
    ...
    xns = eliminacion.eliminacionGaussianaTotal((n * 4), Ab)
    if rank == 0:
        m = [chr(97 + j) + str(i) for i in range(1, n + 1) for j in range(4)]
...
x = [2, 3, 5]
y = [-1, 2, -7]
vars = cubicoNatural(3, x, y)
if rank == 0:
    hallarValor(x, vars, 5)
